 ...
 * heuristic bot, expecting a >=65% win rate (docs/PLANO.md). Not part of `nx test` — at
 * ~20-30s per game this takes hours, so it's a standalone script. Run with:
 *   npx tsx client/tools/run-ismcts-validation.ts [gameCount] [budgetMs]
 */
import { playGame, heuristicBot, makeIsmctsBot } from '../libs/backend-infrastructure/src/index.js';
