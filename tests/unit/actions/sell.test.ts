...
import type { SellAction } from '../../../src/engine/action-types.js';
...
describe('applySell', () => {
  it('sells a cotton mill connected to a matching merchant, flipping it and consuming beer from own brewery', () => {
    ...
    if (coventry === undefined || coventry.kind === 'market') throw new Error('unreachable');
    expect(coventry.slots[0]?.tile).toMatchObject({ flipped: true });
  ...
  it('rejects selling a tile the player does not own', () => {
    ...
    state = {
      ...state,
      locations: withMerchant(
        withTile(state.locations, 'coventry', 0, tile('p2', 'cotton', 1, 0)),
        'oxford',
        0,
        'cotton',
      ),
  ...
  it('rejects selling an already-flipped tile', () => {
    ...
    state = {
      ...state,
      locations: withMerchant(
        withTile(state.locations, 'coventry', 0, tile('p1', 'cotton', 1, 0, true)),
        'oxford',
        0,
        'cotton',
      ),
  ...
  it('sells multiple tiles in one action', () => {
    ...
    const coventry = result.locations['coventry'];
    if (coventry === undefined || coventry.kind === 'market') throw new Error('unreachable');
    expect(coventry.slots[0]?.tile?.flipped).toBe(true);
    expect(coventry.slots[1]?.tile?.flipped).toBe(true);
  });
});
